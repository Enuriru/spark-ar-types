...
/**
```
(get) modeValue: SceneModule.RenderMode
(set) modeValue: SceneModule.RenderMode
```
...
**See Also**: `Canvas.setModeValue`.
*/
modeValue: SceneModule.RenderMode
...
/**
```
(get) useSafeAreaMarginsValue: boolean
(set) useSafeAreaMarginsValue: boolean
```

Specifies if Canvas should automatically include SafeArea margin to its content.
**Note:** this is only effective when `renderMode` property is set to SCREEN_SPACE.
*/
useSafeAreaMarginsValue: boolean
...
/**
```
setModeValue(v: SceneModule.RenderMode, config?: {fallback: SceneModule.RenderMode}): void
```
